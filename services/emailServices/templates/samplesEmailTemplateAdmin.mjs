 ...
 */
export function samplesEmailTemplateAdmin({
  order = {},
  orderId,
  orderDate,
  ...
  const subject = buildAdminPendingPaymentSubjectSample({
    date: formattedDate,
    customerName,
...
export function buildAdminPendingPaymentSubjectSample({ date, customerName, orderId }) {
  ...
  return `Sample Order — ${safeName} (Order ${safeId}, ${safeDate})`;
}
