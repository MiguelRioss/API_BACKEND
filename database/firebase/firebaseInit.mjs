 ...
 * - If FIREBASE_SERVICE_ACCOUNT_PATH exists on disk -> read file and JSON.parse
 * - Else if FIREBASE_SERVICE_ACCOUNT_JSON present -> try JSON.parse as-is
...
function loadServiceAccountSafe() {
  ...
  // 1) explicit JSON file (preferred for rotated keys)
  if (saPath && fs.existsSync(saPath)) {
    try {
      ...
      // keep falling through to env-based fallbacks
    }
  }
  ...
  if (b64Env) {
    try {
      ...
      // If base64 decode or parse fails, keep going to try other fallbacks.
